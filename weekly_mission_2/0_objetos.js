console.log("Objetos")

// Ejemplo 1: Crear un ojeto acío
const myObject = {} //Esto s un objeto vacío
console.log("Ejemplo 1: Crear un objeto vacío")
console.log(myObject)

// Ejemplo 2: Crear un objeto con propiedades
const myObject2 = {
    name: "Eduardo",
    age: 23
}
console.log("Ejemplo 2: Crear un objeto con propiedades")
console.log(myObject2)

//Ejemplo 3: Objeto con diferentes propiedades
const myObject3 = {
    name: "Tulio",
    age: 2,
    nickname: [
        "Tulipan",
        "Tulancingo",
        "Tulish"
    ],
    address: {
        zip_code: "10000",
        street: "Dr. Vertiz 11 Benito Juarez",
        city: "CDMX"
    }
}
console.log("Ejemplo 3: Objetos con diferentes propiedades")
console.log(myObject3)
console.log(myObject3.name)
console.log(myObject3["address"])

// Ejemplo 4: Objeto con métodos
const pet = {
    name:"Tulio",
    //Esta es una función que se fuarda como propiedad
    sayHello: function(){
        //this.name hace referencia a la porpiedad del objeto
        console.log(`${this.name} te saluda en español!`)
    }
}

console.log("Ejemplo 4 con métodos")
pet.sayHello()

// Ejemplo 5: Objeto con método que recie parámtros
const myPet = {
    name: "Woopa",
    sayHelloToMyPet: function(yourPet){
        console.log(`${this.name} say's hello to ${yourPet}`)
    }
}

console.log("Ejemplo 5: Objeto con método que recibe parámetros")
myPet.sayHelloToMyPet("Tulio")
